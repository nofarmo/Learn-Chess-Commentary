...
Original file is located at
    https://colab.research.google.com/drive/1uPgYiYNCMRXApzK0Ja6gTjM1_CnuEAWV
...
loss = 0
pad_token_id = gpt2.tokenizer('[PAD]')['input_ids'][0]
    ...
    with tqdm(total=len(train_dataset) / 2) as pbar:
        for idx,entry in enumerate(train_dataloader):
            ...
            if idx % 2000 == 0 and idx != 0:
              for i in range(config['batch_size']):
                with torch.no_grad():
                    outputs = gpt2.model.generate(validation_input_encodings[i], num_beams=2, no_repeat_ngram_size=2, max_length=769, pad_token_id=pad_token_id)
